...
import { DATA_UNAVAILABLE, ethereumStats, yieldPercentagePerPairs } from '../yam/lib/constants';
...
export const formatNaNWithCommas = num => (isNaN(num) ? DATA_UNAVAILABLE : numberWithCommas(num));

export const formatNaN = num => (isNaN(num) ? DATA_UNAVAILABLE : num);

export const generateRandomSeed = n => {
  const add = 1;
  let max = 12 - add;

  if (n > max) {
    return generateRandomSeed(max) + generateRandomSeed(n - max);
  }
...
export const getContractEventInterface = (yam, contract, eventName) => {
  const eventInterface = yam.web3.utils._.find(
 ...
 */
export const extractTokenNameFromArbitrageStrategyName = (name, token0Out) => {
    // token 0 is always first, we split on /
    let splitName = name.split("/");

    // we split on space
    // "token1 price too low. In "token0" ,"token1", "pair"
    //                         last-2       last-1
    splitName = [...splitName[0].split(" "), ...splitName[1].split(" ")];

    if (!token0Out?.[0]) {
      // If its not token0 out
      // token its token0 cause we want opposite
      return splitName[splitName.length - 3]
    } else {
